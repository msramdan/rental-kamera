...
class Login extends CI_Controller
{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->M_login->checklogin($_POST['username'], $_POST['password']) > 0) {
				...
				if ($this->session->userdata('is_aktive') == 1) {
					...
					$this->session->set_userdata('user_id', $user->anggota_id);
					$this->session->set_userdata('img_user', $user->img_user);
					$this->session->set_userdata('level', $user->level);
					$this->M_login->addHistory($this->session->userdata('username'), $this->session->userdata('username') . ' Telah melakukan login', date('d/m/Y H:i:s'));
					redirect('home');
				} else {
					echo "<script>
	...
	{
		$params = array('username', 'level','user_id','img_user');
